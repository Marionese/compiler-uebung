/***************************************************************************//**
 * @file ast.h
 * @authors Lukas Markeffsky, Dorian Weber
 * @brief Der *abstrakte Syntaxbaum* (AST) von C1.
 *
 * # Überblick
 *
 ...
 * zunächst in eine andere Zwischenrepräsentation umzuwandeln. Daher benötigen
 * wir in bestimmten AST-Knoten zusätzlichen Platz, um zu speichern, auf welche
...
/**
 * Der Datentyp eines Ausdrucks: Einer von `void`, `bool`, `int`, `float` oder
 * `string`.
 * 
 * Zu beachten ist, dass `void` syntaktisch (jedoch nicht semantisch) als
 * Datentyp von Variablen zulässig ist.
 */
typedef enum {
	TYPE_VOID,
	TYPE_BOOL,
 ...
 * zu unterscheiden. Die Kennung wird während der Analyse verwendet.
 * 
 ...
 * 
 * Jede `ItemId` korrespondiert mit dem Index eines Elements im AST.
 */
typedef struct {
	unsigned int index;
} ItemId;
...
/**
 * Erstellt ein Literal aus einer Zeichenkette.
 */
extern Literal astLiteralFromString(char *value);
...
/**
 * Erstellt eine Print-Anweisung aus einer Liste von Ausdrücken.
 * 
 * @param expressions Vektor von Ausdrücken
 * @return Ein neues `PrintStmt`-Objekt.
 */
extern PrintStmt astPrintStmtNew(Expr *expressions);
